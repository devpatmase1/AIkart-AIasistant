...
def get_chroma_client() -> chromadb.ClientAPI:
    ...
    if _chroma_client is None:
        try:
            logger.info(f"Connecting to ChromaDB HTTP server at {settings.CHROMA_HOST}:{settings.CHROMA_PORT}")
            ...
            client.heartbeat()
            logger.info("Connected to ChromaDB HTTP server successfully")
            _chroma_client = client
        except Exception as e:
            chroma_dir = str(settings.BASE_DIR.parent / "chroma_data")
            logger.warning(f"Could not connect to ChromaDB HTTP server ({e}). Falling back to local PersistentClient at {chroma_dir}")
            ...
            logger.info(f"Connected to local ChromaDB PersistentClient at {chroma_dir} successfully")
